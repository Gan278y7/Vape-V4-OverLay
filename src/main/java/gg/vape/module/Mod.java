...
EventListener {
    ...
    public void setEnabled(boolean enabled, boolean bypassVisibilityCheck) {
        ...
        boolean overlayFakeModule = Vape.OVERLAY_ONLY && this.getCategory() != Category.NONE && !(this instanceof HudModule);
        if (!enabled && !overlayFakeModule) {
            this.onBeforeDisable();
        }
        this.enabled = enabled;
        if (this.enabled) {
            if (!overlayFakeModule) {
            ...
            if (!this.enabled && !overlayFakeModule) {
